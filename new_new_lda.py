...
class LDA():
    def __init__(self, Z=10, R=30, n_iteration=5000):
        print 'Fetching data...'
        ...
        self.cnt_topic_word = np.zeros((Z, self.n_word))
        self.cnt_doc_topic = np.zeros((self.n_doc, self.Z))
        ...
        for d in range(self.n_doc):
            for w in self.doc[d, :].nonzero()[1]:
                b = np.random.binomial(1, self.gamma)
                ...
                if b == 0:
                    self.cnt_doc_topic[d, z] += 1
                    self.cnt_topic_word[z, w] += 1
    ...
    def sample_topic(self, d, w):
        ...
        if cur_b == 0:
            self.cnt_topic_word[cur_z, w] -= 1
            self.cnt_doc_topic[d, cur_z] -= 1



        if self.cnt_topic_word[cur_z, w] < 0:
            print 'ERROR! self.cnt_topic_word', self.cnt_topic_word[cur_z, w]
        if self.cnt_doc_topic[d, cur_z] < 0:
        ...
        tmp[0] = p0 * (self.cnt_word_x[w, 1] + self.beta2) /(self.cnt_word_x[:, 1].sum() + self.beta2*self.n_word)

        for k in range(self.Z):
            tmp[1 + k] = (1-p0)*(self.cnt_doc_topic[d, k] + self.alpha) * (self.cnt_topic_word[k, w] + self.beta) / (
                (self.cnt_doc_topic[d, :].sum() + self.alpha * self.Z) * (
                    self.cnt_topic_word[k, :].sum() + self.beta * self.n_word)
            )
    ...
    def sample_indicator(self, d, w):
        ...
        tmp[0] = (1-p0) * (self.cnt_doc_topic[d, cur_z] + self.alpha)*(self.cnt_topic_word[cur_z, w] + self.beta)/(
                (self.cnt_doc_topic[d, :].sum() + self.alpha*self.Z)*(
                    self.cnt_topic_word[cur_z, :].sum() + self.beta*self.n_word)
            )
        ...
        self.x_word_in_doc[(d, w)] = new_b
        self.cnt_doc_x[d, new_b] += 1   # have to add anyway
        self.cnt_word_x[w, new_b] += 1
        if new_b == 0:
            self.cnt_doc_topic[d, cur_z] += 1
            self.cnt_topic_word[cur_z, w] += 1
        ...
        for iteration in range(self.n_iteration):
            ...
            for d in range(self.n_doc):
                if d % 100 == 0:
                    print 'in doc ', d
                for w in self.doc[d, :].nonzero()[1]:
                    self.sample_topic(d, w)
                    #self.sample_indicator(d, w)
